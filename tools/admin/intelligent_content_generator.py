...
class IntelligentContentGenerator:
        ...
        with sqlite3.connect(self.db_path) as conn:
            for element in story_elements:
                element_id = hashlib.md5(
                    f"{element['type']}_{element['content']}".encode()
                ).hexdigest()
                conn.execute(
                    """
    ...
    def _load_economy_data(self):
        ...
        for item in items:
            ...
            price_modifier = (
                1.0 / supply_demand_ratio if supply_demand_ratio > 0 else 1.0
            )
    ...
    def generate_quest(
        self, template_id: str, target_level: int, theme: Optional[ContentTheme] = None
        ...
        try:
            ...
            content_id = (
                f"quest_{template_id}_{int(time.time())}_{random.randint(1000, 9999)}"
            )
            ...
            self.logger.info(
                f"Generated quest: {generated_content.title} (Quality: {quality_score:.2f})"
            )
    ...
    def _generate_quest_variables(
        ...
        for var in template.variables:
            if var == "item_count":
                variables[var] = str(
                    random.randint(1, min(10, max(1, target_level // 10)))
                )
            elif var == "item_name":
    ...
    def _select_level_appropriate_item(self, level_tier: str) -> str:
        ...
        return random.choice(
            items_by_tier.get(level_tier, items_by_tier["intermediate"])
        )
    ...
    def _select_level_appropriate_location(self, level_tier: str) -> str:
        ...
        return random.choice(
            locations_by_tier.get(level_tier, locations_by_tier["intermediate"])
        )
    ...
    def _select_safe_location(self, level_tier: str) -> str:
        ...
        return random.choice(
            safe_locations.get(level_tier, safe_locations["intermediate"])
        )
    ...
    def _fill_template(
        self, template: ContentTemplate, variables: Dict[str, str]
    ) -> Dict[str, Any]:
        ...
        if "objective" in filled_data:
            ...
            filled_data["description"] = (
                f"A quest that requires you to {filled_data['objective'].lower()}."
            )
    ...
    def _calculate_content_quality(
        self, content_data: Dict[str, Any], template: ContentTemplate
    ) -> float:
    ...
    def _assess_content_originality(
        self, content_data: Dict[str, Any], template: ContentTemplate
    ) -> float:
        ...
        try:
            ...
            for item_id, item in self.economy_items.items():
                ...
                if len(item.price_history) > 5:
                    ...
                    volatility = statistics.stdev(recent_prices) / statistics.mean(
                        recent_prices
                    )
    ...
    def _create_economy_adjustment(
        self,
        item_id: str,
        adjustment_type: str,
        old_value: float,
    ...
    def perform_quality_testing(self, content_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            overall_score = sum(test["score"] for test in test_results.values()) / len(
                test_results
            )
    ...
    def _test_narrative_coherence(self, content_data: Dict[str, Any]) -> Dict:
        ...
        if any(
            key in content_data
            for key in ["challenges", "mystery_subject", "special_conditions"]
        ):
            score += 0.2
        ...
        return {
            ...
            "result": (
                "pass"
                if score >= self.quality_thresholds["narrative_coherence"]
                else "fail"
            ),
            "details": details,
        }

    def _test_gameplay_balance(
        self, content_data: Dict[str, Any], content_type: str
    ) -> Dict:
        ...
        return {
            ...
            "result": (
                "pass"
                if score >= self.quality_thresholds["gameplay_balance"]
                else "fail"
            ),
            "details": details,
        }
    ...
    def _test_technical_feasibility(self, content_data: Dict[str, Any]) -> Dict:
        ...
        return {
            ...
            "result": (
                "pass"
                if score >= self.quality_thresholds["technical_feasibility"]
                else "fail"
            ),
            "details": details,
        }
    ...
    def _test_player_engagement(self, content_data: Dict[str, Any]) -> Dict:
        ...
        # Check for story elements
        if any(
            key in content_data for key in ["mystery_subject", "revelation", "giver"]
        ):
            score += 0.2
        ...
        return {
            ...
            "result": (
                "pass"
                if score >= self.quality_thresholds["player_engagement"]
                else "fail"
            ),
            "details": details,
        }
        ...
        if len(sys.argv) > 1:
            ...
            if command == "start":
                ...
                if len(sys.argv) > 3:
                    ...
                    print(
                        "Usage: intelligent_content_generator.py generate <template_id> <level>"
                    )
            elif command == "test":
                if len(sys.argv) > 2:
                ...
                print(
                    "Usage: intelligent_content_generator.py [start|status|generate|test]"
                )
        else:
            generator.start()
