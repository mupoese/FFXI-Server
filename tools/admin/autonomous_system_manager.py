...
class AutonomousSystemManager:
    ...
    def _load_config(self, config_path: Optional[str]) -> Dict:
        ...
        default_config = {
            ...
            'services_to_monitor': [
                'xi_connect', 'xi_search', 'xi_map', 'xi_world'
            ],
            ...
            if check.status == "critical" and check.auto_fix_available:
                ...
                try:
                    ...
                    self._log_healing_action(
                        check.component, success, action_details
                    )
                    
                except Exception as e:
                    self.logger.error(f"Healing action failed for {check.component}: {e}")
                    self._log_healing_action(
                        check.component, False, f"Error: {str(e)}"
                    )
            ...
            for proc in high_cpu_procs[:3]:  # Limit to top 3
                try:
                    if proc.info['name'] in self.config['services_to_monitor']:
                        self.logger.info(f"Restarting high-CPU process: {proc.info['name']}")
                        proc.terminate()
                        proc.wait(timeout=5)
        ...
        try:
            ...
            for log_dir in log_dirs:
                if os.path.exists(log_dir):
                    subprocess.run([
                        'find', log_dir, '-name', '*.log', '-mtime', 
                        f'+{retention_days}', '-delete'
                    ], capture_output=True, check=False)
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                if len(cpu_data) > 10:
                    ...
                    if recent_avg > older_avg * 1.2:  # 20% increase
                        self.logger.warning("CPU usage trending upward - consider scaling")
    ...
    def monitoring_loop(self):
        ...
        while self.running:
            try:
                ...
                if critical_issues:
                    self.logger.warning(f"Critical issues detected: {len(critical_issues)}")
